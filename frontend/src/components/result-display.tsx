...
// ---------------------------------------------------------------------------
// NewsVerificationPanel — separate component
// ---------------------------------------------------------------------------
...
function NewsVerificationPanel({ nv }: NewsVerificationPanelProps) {
  const [expanded, setExpanded] = useState(false);

  const iconAndColor = (() => {
    if (!nv.available) return { Icon: AlertCircle, color: "text-white/40", border: "border-white/10" };
    if (nv.contradiction_detected) return { Icon: XCircle, color: "text-rose-400", border: "border-rose-500/30" };
    const score = nv.corroboration_score ?? 0;
    if (score >= 0.65) return { Icon: CheckCircle2, color: "text-emerald-400", border: "border-emerald-500/30" };
    if (score >= 0.40) return { Icon: AlertCircle, color: "text-amber-400", border: "border-amber-500/30" };
    return { Icon: XCircle, color: "text-rose-400", border: "border-rose-500/30" };
  })();

  const { Icon, color, border } = iconAndColor;
  ...
  return (
    <div className={`glass rounded-3xl border ${border} p-6 space-y-4`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-bold text-white/80 uppercase tracking-wider">News Verification</h3>
        {nv.source_count > 0 && (
      ...
      <div className={`flex items-center gap-3 p-3 rounded-xl bg-black/30 border ${border}`}>
        ...
        <p className={`text-sm font-semibold ${color}`}>{nv.verification_label || nv.message}</p>
      </div>
      ...
      {nv.corroboration_score !== null && nv.corroboration_score !== undefined && (
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">Corroboration strength</p>
            <p className="text-sm font-black font-mono text-white">{Math.round(nv.corroboration_score * 100)}%</p>
          </div>
      ...
      {nv.available && nv.articles.length > 0 && (
        <div className="space-y-2">
          ...
          >
            {expanded ? "▲" : "▼"} {expanded ? "Hide" : "Show"} matched articles ({nv.articles.length})
          </button>
...
function ArticleCard({ article }: { article: NewsArticle }) {
  ...
  const date = (() => {
    ...
    try {
      return new Date(article.published_at).toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" });
    } catch {
      return null;
    }
  })();
    ...
    <div className="rounded-xl border border-white/8 bg-white/[0.02] p-3 space-y-1.5">
      <div className="flex items-start gap-2">
        ...
        {article.url ? (
          ...
          <p className="text-xs font-semibold text-white/80 line-clamp-2 leading-snug">{article.title}</p>
        )}
      </div>
      {article.description && (
        <p className="text-[11px] text-white/55 line-clamp-2 leading-relaxed pl-5">{article.description}</p>
      )}
      <div className="flex items-center gap-2 pl-5">
        <span className="text-[10px] font-bold text-white/40 uppercase tracking-wide">{domain}</span>
