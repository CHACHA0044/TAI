...
class InferenceEngine:
    ...
    def _load_model(self, model_path):
        ...
        if HAS_TRANSFORMERS and TruthGuardMultiTaskModel:
            try:
                ...
                # Load model
                # If using local path, we might need to load the weights manually or rely on from_pretrained
    ...
    def analyze(self, text_or_url):
        ...
        # 5. Fusion & self-check
        # When no external signals are available (verification_signals is empty)
        # avg_external_cred defaults to 0.5 (neutral), which previously dragged
        # truth_score down from ~0.55 to ~0.53 — but TrustAgent mock headlines
        # were off-topic causing 0.2 credibility which was the real culprit.
        # Now we only apply external credibility if signals actually ran.
        if verification_signals:
            final_truth_score = self._sanitize((truth_score * 0.6) + (avg_external_cred * 0.4))
        else:
            final_truth_score = self._sanitize(truth_score)

        # Only penalise if both AI probability is very high AND external credibility
